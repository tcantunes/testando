...
import { atualizarVaga } from "../lib/backend";
...
type EditarVagaRouteProp = RouteProp<StackParamList, "EditarVaga">;
type NavigationProp = StackNavigationProp<StackParamList, "OngAcoes">;

const VagaSchema = Yup.object().shape({
  nome: Yup.string().required("Informe o nome da vaga"),
  descricao: Yup.string().required("Informe a descrição"),
  local: Yup.string().required("Informe o local"),
  data_hora: Yup.date().required("Informe a data e hora").nullable(),
  vagas_disponiveis: Yup.number()
...
const EditarVagaScreen = () => {
  const route = useRoute<EditarVagaRouteProp>();
  const navigation = useNavigation<NavigationProp>();
  const { vaga } = route.params;
  const [loading, setLoading] = useState(false);
  const [loadingLocation, setLoadingLocation] = useState(false);

  const handleEditarVaga = async (values: any) => {
    setLoading(true);
    ...
    try {
      ...
      await atualizarVaga(vaga.id, payload);

      Alert.alert("Sucesso! 🎉", "Ação atualizada com sucesso!", [
        {
          text: "OK",
          onPress: () => navigation.navigate("OngAcoes" as never),
        },
      ]);
    } catch (error: any) {
      Alert.alert(
        "Erro",
        error?.response?.data?.error || "Não foi possível atualizar a ação."
      );
    } finally {
      setLoading(false);
    }
    ...
    <SafeAreaView style={styles.container} edges={['bottom']}>
      ...
      <Formik
        initialValues={{
          ...
          local: vaga.local || "",
          data_hora: vaga.data_hora ? new Date(vaga.data_hora) : null as Date | null,
          vagas_disponiveis: String(vaga.vagas_disponiveis || 1),
          categoria: vaga.categoria || "",
          cep: vaga.cep || "",
          cidade: vaga.cidade || "",
          estado: vaga.estado || "",
          latitude: vaga.latitude || null as number | null,
          longitude: vaga.longitude || null as number | null,
        }}
          ...
          >
            <View style={styles.form}>
              ...
              <Input
                ...
                placeholder="Ex: Plantio de árvores"
                value={values.nome}
                onChangeText={handleChange("nome")}
                icon="title"
                error={touched.nome && errors.nome ? String(errors.nome) : undefined}
              />
              ...
              <Input
                ...
                icon="home"
                error={touched.local && errors.local ? String(errors.local) : undefined}
              />
...
export default EditarVagaScreen;
